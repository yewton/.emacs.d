...
インストールしたテーマを設定します。
[[https://github.com/oneKelvinSmith/monokai-emacs][monokai]] テーマを使います:
...
#+BEGIN_SRC emacs-lisp
  (use-package swiper
    ...
    (setq open-junk-file-format (expand-file-name "junk/%Y/%m/%d-%H%M%S." my-data-directory))
  ...
  When ARG is non-nil search in junk files."
    ...
    (bind-key "C-x C-z" #'my-open-junk-file)
...
絵文字を表示出来るようにします :blush:

~emojify-insert-emoji~ で ivy 補完しながら絵文字を入力出来る。
...
#+BEGIN_SRC emacs-lisp
  (use-package browse-at-remote
    :init (setq browse-at-remote-prefer-symbolic nil))
#+END_SRC
