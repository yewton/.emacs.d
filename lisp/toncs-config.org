...
(defconst toncs-config-font-candidates `(("PlemolJP" . 13.0)
                                         ("Noto Sans Mono" . 14.0)))
...
(defun toncs-config-font-configure (&optional reload)
  ...
  (let* ((font-specs (mapcar (lambda (elt) (font-spec :family (car elt) :size (cdr elt)))
    ...
    (when (fboundp 'set-fontset-font)
      (set-fontset-font t 'symbol font-spec) ;; 記号系もデフォルトはメインのフォントにする
    ...
    (dolist (face '(default fixed-pitch))
      (custom-set-faces
       `(,face
         ((t (:family ,font-spec :weight unspecified :slant unspecified :width unspecified))))))
    (cond
     (font-spec
      (if reload
...
(toncs-config-configure helpful
  ;; see https://github.com/Wilfred/elisp-refs/issues/35
  (general-def
