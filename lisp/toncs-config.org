...
#+begin_src emacs-lisp
(defconst toncs-config-font-candidates `(("HackGen" . ,(pcase (window-system)
                                                         ('ns 13)
                                                         (_ 17)))
                                         ("Noto Sans Mono" . 14)))
...
#+begin_src emacs-lisp
(require 'shackle)
(defvar dirvish-display-buffer-alist)
(defvar dirvish-saved-display-buffer-alist)

(defun dirvish-update-display-buffer-alists ()
  (setq dirvish-display-buffer-alist
        (append '(("^\\*F\\(?:d\\|ind\\)\\*$" (display-buffer-same-window)))
...
(toncs-config-configure shackle
  ...
  (add-hook 'shackle-mode-hook #'dirvish-update-display-buffer-alists))
#+end_src
