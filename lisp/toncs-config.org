...
(autoload 'toncs-org-node-search "toncs-config-org-node")
(autoload 'toncs-org-node-goto-today "toncs-config-org-node")
(autoload 'toncs-org-node-goto-date "toncs-config-org-node")
(autoload 'toncs-org-node-capture "toncs-config-org-node")
(autoload 'toncs-org-node-capture-ref "toncs-config-org-node")
(toncs-config-prepare org-node
  (general-def
    "C-c n f" #'org-node-find
    "C-c n t" #'toncs-org-node-goto-today
    "C-c n d" #'toncs-org-node-goto-date
    "C-c n T" #'org-node-seq-dispatch
    "C-c n c" #'toncs-org-node-capture
    "C-c n r" #'toncs-org-node-capture-ref
    "C-c n s" #'toncs-org-node-search)
...
(defconst toncs-config-font-candidates '(("PlemolJP Console NF" . 12.0)
                                         ("Noto Sans Mono" . 14.0)))

(defconst toncs-config-vp-font-candidates '(("PlemolJP35" . 12.0)
...
(defun toncs-config-font-configure (&optional reload)
  ...
  (let* ((font-specs (mapcar (lambda (elt) (font-spec :family (car elt) :size (cdr elt)))
    ...
    (when (fboundp 'set-fontset-font)
      ...
      ;; サイズをメインフォントと揃えることで行高ガタつきを防ぐ
      (let ((size (font-get font-entity :size)))
        (dolist (font (list emojifont symbolsfont))
          (when font
            (set-fontset-font t 'symbol (font-spec :family font :size size) nil 'append)))))
    (when font-entity
...
(defun toncs-server-prompt-root ()
  "emacsclient 接続時に送られてくる `-dir' (呼び出し元 cwd) を返す。"
  (and server-buffer-clients
       (process-get (car server-buffer-clients) 'server-client-directory)))

(defun toncs-claude-command-names-in (dir)
  "DIR (.claude/commands 相当) 以下の *.md からコマンド名一覧を作る。"
...
(defun toncs-claude-skill-names-in (dir)
  "DIR (.claude/skills 相当) 直下から SKILL.md を持つスキル名一覧を作る。"
  (when (file-directory-p dir)
    (seq-filter
     (lambda (name)
...
(defun toncs-claude-command-candidates ()
  "プロジェクト・ユーザ双方から利用可能なスラッシュコマンド名一覧を返す。"
  (let ((root (or (toncs-server-prompt-root) default-directory)))
    (delete-dups
     (append
      (toncs-claude-command-names-in (expand-file-name ".claude/commands" root))
      (toncs-claude-command-names-in (expand-file-name "~/.claude/commands"))
      (toncs-claude-skill-names-in (expand-file-name ".claude/skills" root))
      (toncs-claude-skill-names-in (expand-file-name "~/.claude/skills"))))))

(defun toncs-claude-command-capf ()
  "バッファ先頭が / で始まる場合にスラッシュコマンド名を補完する。"
  (when (save-excursion (goto-char (point-min)) (looking-at-p "/"))
    (let* ((beg (1+ (point-min)))
           (end (point)))
      (when (and (<= beg end)
                 (not (string-match-p "[ \t\n]"
                                       (buffer-substring-no-properties beg end))))
        (list beg end (toncs-claude-command-candidates) :exclusive 'no)))))

(defun toncs-server-prompt-edit-setup ()
  "See https://stackoverflow.com/a/268205/2142831 ."
  (when-let* ((file buffer-file-name)
              (kind (cond
                     ((string-match-p "/gemini-edit-.+/buffer\\.txt\\'" file) 'gemini)
                     ((string-match-p "/copilot-edit-.+/COPILOT_PROMPT\\.md\\'" file) 'copilot)
                     ((string-match-p "/claude-prompt-.+\\.md\\'" file) 'claude)
                     ((string-match-p "/qwen-edit-.+/buffer\\.txt\\'" file) 'qwen)
                     ((string-match-p "/jetski-prompt-.+\\.txt\\'" file) 'jetski))))
    (general-def 'local
      "C-c C-c" (lambda () (interactive) (save-buffer) (kill-buffer (current-buffer)))
      "C-c C-k" (lambda () (interactive) (set-buffer-modified-p nil) (kill-buffer (current-buffer))))
    ...
    (add-hook 'kill-buffer-hook
              ...
              nil 'local)
    ;; ファイル名補完をポップアップ元 (プロジェクトルート) 基準にする
    (setq-local cape-file-directory
                (lambda () (or (toncs-server-prompt-root) default-directory)))
    (setq-local cape-file-prefix (cons "@" (ensure-list cape-file-prefix)))
    (when (eq kind 'claude)
      (add-hook 'completion-at-point-functions #'toncs-claude-command-capf nil t))))

(toncs-config-configure server
  (add-hook 'server-visit-hook #'toncs-server-visit-hook-function)
  (setq server-window #'pop-to-buffer)
  (add-to-list 'auto-mode-alist
             '("/gemini-edit-.*/buffer\\.txt\\'" . markdown-mode))
  (add-to-list 'auto-mode-alist
             '("/qwen-edit-.*/buffer\\.txt\\'" . markdown-mode))
  (add-to-list 'auto-mode-alist
             '("/jetski-prompt-.+\\.txt\\'" . markdown-mode))
  (add-hook 'markdown-mode-hook #'toncs-server-prompt-edit-setup)
...
(toncs-config-configure shackle
  (dolist (rule `((compilation-mode :select nil :align below :size 0.2)
                  ...
                  (org-node-context-mode :select nil :align right :size 0.33 :popup t)
                  ...
                  ("/copilot-edit-.+/COPILOT_PROMPT\\.md\\'" :regexp t :select t :align below :popup t)
                  ("/claude-prompt-.+\\.md\\'" :regexp t :select t :align below :popup t)
                  ("/qwen-edit-.+/buffer\\.txt\\'" :regexp t :select t :align below :popup t)
...
(toncs-config-configure project
  ...
  ;; projectile-vc の手癖を引き継ぎ、素の vc-dir ではなく magit を開く
...
#+begin_src emacs-lisp
(defun toncs-side-panel--toggle (side alist)
  "SIDE のサイドウィンドウにカレントバッファを表示／削除する。"
  (let ((win (get-window-with-predicate
              (lambda (w) (eq (window-parameter w 'window-side) side)))))
    (if win
        (delete-window win)
      (let ((buf (current-buffer))
            (w (display-buffer-in-side-window
                (current-buffer)
                `((side . ,side) (slot . 0) ,@alist))))
        (set-window-parameter w 'no-delete-other-windows t)
        (set-window-dedicated-p w buf)
        (select-window w)))))

(defun toncs-side-panel-bottom ()
  "ボトムパネルをトグルする。"
  (interactive)
  (toncs-side-panel--toggle 'bottom '((window-height . 0.3))))

(defun toncs-side-panel-right ()
  "ライトパネルをトグルする。"
  (interactive)
  (toncs-side-panel--toggle 'right '((window-width . 0.4))))
#+end_src
...
前置することで両立させる（ghostel README の「Picker annotations」節に
記載のレシピ）。

#+begin_src emacs-lisp
(require 'cl-lib)

(toncs-config-prepare ghostel)

(declare-function ghostel-marginalia-annotate-buffer nil)

(toncs-config-configure ghostel
...
~xref-backend-functions~ は ~run-hook-with-args-until-success~ で評価されるため、
バッファローカルな専用バックエンド（elisp-mode 等）が優先され、dumb-jump は
それらが対応しない場合にのみ使われる。 ~add-hook~ の既定（prepend）で登録するのは、
グローバルに登録される ~etags--xref-backend~ （常に非 nil を返す）より前に置くため。
append にすると dumb-jump が etags に永久に隠れてしまう。
