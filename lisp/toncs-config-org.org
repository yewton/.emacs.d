...
(defun toncs-org-read-date-backward () (interactive) (org-funcall-in-calendar '(calendar-backward-day 1)))
(defun toncs-org-read-date-next () (interactive) (org-funcall-in-calendar '(calendar-forward-week 1)))
(defun toncs-org-read-date-previous () (interactive) (org-funcall-in-calendar '(calendar-backward-week 1)))
(defun toncs-org-read-date-forward () (interactive) (org-funcall-in-calendar '(calendar-forward-day 1)))
#+end_src
...
(defun toncs-config-org-clock-configure ()
  ...
  (general-def 'override
    ...
    "C-c C-x C-g" #'org-mru-clock-goto
...
(defcustom toncs-org-download-attach-exclude-from-roam nil
  "非nilなら、org-download が画像添付のために新規に :ID: を作った
見出しに、org-node の補完候補から外れるよう自動で :ROAM_EXCLUDE: t
を付与する。呼び出し前に既に :ID: がある見出しに画像を貼る場合は
その見出しが元々意味のある node である可能性があるため、何もしない。

デフォルトは無効。有効にするには (setq toncs-org-download-attach-exclude-from-roam t)
を `custom-file'（例: ~/.emacs.local/init.el）に書く。"
  :type 'boolean
  :group 'toncs)

(defun toncs-org-download--exclude-new-attach-node (orig-fn &rest args)
  "`toncs-org-download-attach-exclude-from-roam' が非nilのとき、
ORIG-FN 呼び出し前に :ID: が無ければ、呼び出し後に :ROAM_EXCLUDE: t を付与する。"
  (if (not toncs-org-download-attach-exclude-from-roam)
      (apply orig-fn args)
    (let ((had-id (org-entry-get nil "ID")))
...
(defun toncs-config-org-org-download-configure ()
  ...
  (advice-add 'org-download-clipboard :around #'toncs-org-download--exclude-new-attach-node)
  (advice-add 'org-download-image :around #'toncs-org-download--exclude-new-attach-node))
#+end_src
